...
function revealAllMines(){
    for(var i = 0; i < gameBoardWidthAndHeight.width; i++){
        for(var j = 0; j < gameBoardWidthAndHeight.height; j++){
            if(gameBoard[i][j].mine === true && gameBoard[i][j].flag === false)
                $("#" + i +"t"+ j).addClass("clicked mine fa fa-bomb");   
            else if(gameBoard[i][j].mine === false && gameBoard[i][j].flag === true){
                $("#" + i +"t"+ j).removeClass("fa-flag");                
                $("#" + i +"t"+ j).addClass("clicked fa-times fa-lg");
            }
        }
    }    
};
